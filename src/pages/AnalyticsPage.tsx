      ...
      datasets: [
        {
          ...
          backgroundColor: [chartTheme.green, chartTheme.rose],
          borderColor: chartTheme.surface,
          borderWidth: 4,
        },
      ],
    };
  }, [chartTheme.green, chartTheme.rose, chartTheme.surface, overview]);
      ...
      <section className="hero-surface">
        <div className="mb-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className="page-header mt-1">Analytics</h1>
          ...
          <button
            ...
            className={`rounded-xl px-4 py-2 text-sm font-medium transition ${
              isSyncRunning || isBlockedByAnotherYear
                ...
                : "bg-[hsl(var(--accent-cyan))] text-black shadow-lg shadow-cyan-500/20 hover:brightness-110"
            }`}
          >
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm font-medium transition ${
                selectedYear === year.value
                  ? "bg-[hsl(var(--accent-violet))] text-white shadow-lg shadow-violet-500/20"
            ...
            {loadingOverview || !overview || syncRefreshing ? (
              ...
              <>
                <div className="metric-tile">
                  ...
                  <h2 className="mt-3 text-3xl font-bold text-foreground">{overview.totalStudents}</h2>
                </div>
                <div className="metric-tile">
                ...
                <div className="metric-tile">
                  <p className="text-sm text-muted-foreground">{overview.averageLabel}</p>
            ...
            <>
              <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
                <div className="chart-panel">
                  <div className="mb-5">
                    <h3 className="text-lg font-semibold text-foreground">Department Pass Rate</h3>
                ...
                <div className="chart-panel">
                  <div className="mb-5">
                    <h3 className="text-lg font-semibold text-foreground">Pass / Fail Split</h3>
                    <p className="text-sm text-muted-foreground">
                  ...
                  (student, index) => {
                    ...
                    return (
                      <div
                        ...
                        className="metric-tile text-center"
                      >
                        {loadingOverview || syncRefreshing ? (
                          ...
                          <>
                            ...
                            <h3 className="mt-2 font-semibold text-foreground">{typedStudent.name}</h3>
                            <p className="text-sm text-muted-foreground">{typedStudent.roll_number}</p>
              ...
              <div className="data-card">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-semibold text-foreground">Top Students</h3>
                    <p className="text-sm text-muted-foreground">
                ...
                {loadingOverview || syncRefreshing ? (
                  ...
                  <p className="text-muted-foreground">No student rankings are available for this selection.</p>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {topStudents.map((student) => (
                          <tr key={student.roll_number} className="border-b border-border/50">
      ...
      {view === "department" && (
        <>
              ...
              <button
                ...
                className={`rounded-xl px-4 py-2 text-sm font-medium transition ${
                  selectedDept === department.id
                    ? "bg-[hsl(var(--accent-cyan))] text-black shadow-lg shadow-cyan-500/20"
                    : "bg-white/[0.05] text-muted-foreground hover:bg-white/[0.08]"
                }`}
              >
                {department.name}
          ...
          {loadingDepartment || !departmentDetails || syncRefreshing ? (
            ...
            <div className="data-card">
              <h3 className="text-lg font-semibold text-foreground">No Department Analytics Available</h3>
              <p className="mt-2 text-muted-foreground">
            ...
            <>
              ...
              <div className="grid gap-6 xl:grid-cols-[1fr_1.2fr]">
                <div className="data-card">
                  <h3 className="mb-4 text-lg font-semibold text-foreground">Top in Department</h3>
                  {departmentDetails.topStudents.length === 0 ? (
                    <p className="text-muted-foreground">No department toppers available.</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <tbody>
                ...
                {mode === "current" ? (
                  <div className="data-card">
                    <h3 className="mb-4 text-lg font-semibold text-foreground">Subject Analysis</h3>
                    {departmentDetails.subjects.length === 0 ? (
                      <p className="text-muted-foreground">No subject analysis available.</p>
                    ) : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead className="border-b border-border/60 text-muted-foreground">
                          ...
                          <tbody>
                            {departmentDetails.subjects.map((subject) => (
                              <tr key={subject.subject_name} className="border-b border-border/50">
                  ...
                  <div className="data-card">
                    <h3 className="mb-4 text-lg font-semibold text-foreground">Mode Insight</h3>
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                {(Object.entries(departmentDetails.classes) as [string, SectionSummary][]).map(([section, sectionData]) => (
                  <div key={section} className="data-card p-5">
                    <h3 className="mb-3 text-lg font-semibold text-foreground">Section {section}</h3>
                    ...
                    {sectionData.topStudents.length === 0 ? (
                      <p className="text-muted-foreground">No ranked students in this section yet.</p>
                    ) : (
                      <table className="w-full text-sm">
                        <tbody>
                          {sectionData.topStudents.map((student) => {
                            const typedStudent = student as TopStudent;
                            return (
                                <tr key={typedStudent.roll_number} className="border-b border-border/50">
